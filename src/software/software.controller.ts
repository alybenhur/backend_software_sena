...
@UseInterceptors(FilesInterceptor('imagenes', 10))
@ApiConsumes('multipart/form-data')
@ApiOperation({ summary: 'Agregar imágenes a un software existente (Solo Super Admin)' })
@ApiResponse({ status: 200, description: 'Imágenes agregadas exitosamente' })
@ApiResponse({ status: 404, description: 'Software no encontrado' })
uploadImages(
  @Param('id', ParseIntPipe) id: number,
  @UploadedFiles() files: Express.Multer.File[],
) {
  return this.softwareService.addImages(id, files);
}
